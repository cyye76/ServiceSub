...
public class DataEntryHandler4B6 extends Thread
{
	public void run( )
	{
		try
			...
			while(true)
			{
				if(scanInput.hasNext())
				{
					...
					if(dataType.equalsIgnoreCase("SRT"))
					{
					    ...
					    BrokerSix.insertToPubList(pub, rowSRT.tasks);
					    BrokerSix.newPubMatchWithPRT(rowSRT, pub);   
					    //display all the tables contents together
					    BrokerSix.displayAllTable();
						System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: if: SRT
					else if(dataType.equalsIgnoreCase("PRT"))
					{
						...
						BrokerSix.insertToSubList(sub, tasks4SubList);
						BrokerSix.matchWithPRT(rowPRT,false);  
					    //display all the tables contents together
						BrokerSix.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: else if: PRT
