...
public class DataEntryHandler4B5 extends Thread
{
	public void run( )
	{
		try
			...
			while(true)
			{
				if(scanInput.hasNext())
				{
					...
					if(dataType.equalsIgnoreCase("SRT"))
					{
					    ...
					    BrokerFive.insertToPubList(pub, rowSRT.tasks);
					    BrokerFive.newPubMatchWithPRT(rowSRT, pub);   
					    //display all the tables contents together
					    BrokerFive.displayAllTable();
						System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: if: SRT
					else if(dataType.equalsIgnoreCase("PRT"))
					{
						...
						BrokerFive.insertToSubList(sub, tasks4SubList);
						BrokerFive.matchWithPRT(rowPRT,false);  
					    //display all the tables contents together
						BrokerFive.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: else if: PRT
