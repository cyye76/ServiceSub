...
public class DataEntryHandler4B2 extends Thread
{
	public void run( )
	{
		try
			...
			while(true)
			{
				if(scanInput.hasNext())
				{
					...
					if(dataType.equalsIgnoreCase("SRT"))
					{
					    ...
					    BrokerTwo.insertToPubList(pub, rowSRT.tasks);
					    BrokerTwo.newPubMatchWithPRT(rowSRT, pub); 
					    //display all the tables contents together
					    BrokerTwo.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: if: SRT
					else if(dataType.equalsIgnoreCase("PRT"))
					{
						...
						BrokerTwo.insertToSubList(sub, tasks4SubList);
						BrokerTwo.matchWithPRT(rowPRT,false);   
						//display all the tables contents together
					    BrokerTwo.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: else if: PRT
