...
public class DataEntryHandler4B3 extends Thread
{
	public void run( )
	{
		try
			...
			while(true)
			{
				if(scanInput.hasNext())
				{
					...
					if(dataType.equalsIgnoreCase("SRT"))
					{
					    ...
					    BrokerThree.insertToPubList(pub, rowSRT.tasks);
					    BrokerThree.newPubMatchWithPRT(rowSRT, pub);   
					    //display all the tables contents together
					    BrokerThree.displayAllTable();
						System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: if: SRT
					else if(dataType.equalsIgnoreCase("PRT"))
					{
						...
						BrokerThree.insertToSubList(sub, tasks4SubList);
						BrokerThree.matchWithPRT(rowPRT,false);  
					    //display all the tables contents together
						BrokerThree.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: else if: PRT
