...
public class DataEntryHandler4B1 extends Thread
{
	public void run( )
	{
		try
			...
			while(true)
			{
				if(scanInput.hasNext())
				{
					...
					if(dataType.equalsIgnoreCase("SRT"))
					{
					    ...
					    BrokerOne.insertToPubList(pub, rowSRT.tasks);
					    BrokerOne.newPubMatchWithPRT(rowSRT, pub);   
					    //display all the tables contents together
						BrokerOne.displayAllTable();
						System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: if: SRT
					else if(dataType.equalsIgnoreCase("PRT"))
					{
						...
						BrokerOne.insertToSubList(sub, tasks4SubList);
					    BrokerOne.matchWithPRT(rowPRT,false);  
					    //display all the tables contents together
					    BrokerOne.displayAllTable();
					    System.out.print("Enter data type (SRT or PRT?) : ");
					}//END: else if: PRT
